...
todaySalary=$(($hours * $RATE))
                employeeDailyWage[$TOTAL_WORKING_DAYS]="Day$TOTAL_WORKING_DAYS $todaySalary"
#todaysWage=$((rate * $hours)); 
done
TOTAL_SALARY=$(($TOTAL_EMP_HOURS * $RATE));
echo $TOTAL_SALARY;
echo ${employeeDailyWage[@]};  #array
echo ${!employeeDailyWage[@]}  #index
echo "Daily Wages -" ${employeeDailyWage[@]};
